...
df_test = spark.read.format('xml').options(
...
def filter_second(links):
    ...
    get_first_link = list(
        map(lambda link: link.split("|")[0].lower(), ignore_hash))
...
filtered_df_test = ext_df_test.withColumn(
    "filtered", udf_filter_second(col("ext_links"))).select("title", "filtered")
lower_df_test = filtered_df_test.withColumn(
    "lower_title", udf_lower_title(col("title"))).select("lower_title", "filtered")

out_df_test = lower_df_test.select(
    lower_df_test.lower_title, explode(lower_df_test.filtered))
...
out_df_test.write.option("delimiter", "\t").csv(
    "/q2-small")  # type cmd $hadoop fs -ls /
